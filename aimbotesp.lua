...
local espObjects = {}   
local waypointPlayer = nil
local waypointAdornment = nil
local removeWaypoint
local rebuildWaypointList
...
local waypointPanel = createTab("Waypoint", 6)
...
do
makeLabel(waypointPanel, "Waypoint", 1)

local waypointStatus = makeLabel(waypointPanel, "No waypoint set", 2)
waypointStatus.TextColor3 = COL_DIM

local removeWaypointFrame = Instance.new("Frame")
removeWaypointFrame.Size = UDim2.new(1, 0, 0, 40)
removeWaypointFrame.BackgroundColor3 = COL_PANEL_ALT
removeWaypointFrame.BorderSizePixel = 0
removeWaypointFrame.LayoutOrder = 3
removeWaypointFrame.Parent = waypointPanel

local removeWaypointFrameCorner = Instance.new("UICorner")
removeWaypointFrameCorner.CornerRadius = UDim.new(0, 8)
removeWaypointFrameCorner.Parent = removeWaypointFrame

local removeWaypointFrameStroke = Instance.new("UIStroke")
removeWaypointFrameStroke.Thickness = 1
removeWaypointFrameStroke.Color = COL_BORDER
removeWaypointFrameStroke.Transparency = 0.1
removeWaypointFrameStroke.Parent = removeWaypointFrame

local removeWaypointBtn = Instance.new("TextButton")
removeWaypointBtn.Size = UDim2.new(1, -8, 1, -8)
removeWaypointBtn.Position = UDim2.new(0, 4, 0, 4)
removeWaypointBtn.BackgroundColor3 = COL_PANEL
removeWaypointBtn.BorderSizePixel = 0
removeWaypointBtn.Text = "Remove Waypoint"
removeWaypointBtn.Font = Enum.Font.GothamBold
removeWaypointBtn.TextSize = 12
removeWaypointBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
removeWaypointBtn.Parent = removeWaypointFrame

local removeWaypointCorner = Instance.new("UICorner")
removeWaypointCorner.CornerRadius = UDim.new(0, 6)
removeWaypointCorner.Parent = removeWaypointBtn

local waypointList = Instance.new("ScrollingFrame")
waypointList.Size = UDim2.new(1, 0, 0, 220)
waypointList.BackgroundColor3 = COL_PANEL
waypointList.BorderSizePixel = 0
waypointList.LayoutOrder = 4
waypointList.AutomaticCanvasSize = Enum.AutomaticSize.None
waypointList.CanvasSize = UDim2.new(0, 0, 0, 0)
waypointList.ScrollBarThickness = 4
waypointList.Parent = waypointPanel

local waypointListCorner = Instance.new("UICorner")
waypointListCorner.CornerRadius = UDim.new(0, 8)
waypointListCorner.Parent = waypointList

local waypointListStroke = Instance.new("UIStroke")
waypointListStroke.Thickness = 1
waypointListStroke.Color = COL_BORDER
waypointListStroke.Transparency = 0.1
waypointListStroke.Parent = waypointList
...
local function removeWaypointImpl()
    waypointPlayer = nil
    if waypointAdornment then
        waypointAdornment:Destroy()
        waypointAdornment = nil
    end
    waypointStatus.Text = "No waypoint set"
end

local function setWaypointForPlayer(player)
    if not player or player == LocalPlayer then return end
    local character = player.Character
    local head = character and character:FindFirstChild("Head")
    if not head then
        task.spawn(showCustomNotification, "Player is not ready for a waypoint.", 5)
        return
    end
    ...
    bg.BackgroundColor3 = COL_PANEL_ALT
    ...
    bgStroke.Color = COL_ACCENT
    bgStroke.Parent = bg

    local title = Instance.new("TextLabel")
    title.Size = UDim2.new(1, -8, 1, -8)
    ...
    waypointAdornment = billboard
    waypointAdornment.Parent = ScreenGui

    waypointStatus.Text = "Waypoint: " .. getPlayerDisplayName(player)
end

local function rebuildWaypointListImpl()
    ...
    for idx, player in ipairs(candidates) do
        ...
        rowFrame.Size = UDim2.new(1, -8, 0, 42)
        rowFrame.Position = UDim2.new(0, 4, 0, 0)
        rowFrame.BackgroundColor3 = COL_PANEL_ALT
        rowFrame.BorderSizePixel = 0
        rowFrame.LayoutOrder = idx
        rowFrame.Parent = waypointList

        local rowFrameCorner = Instance.new("UICorner")
        rowFrameCorner.CornerRadius = UDim.new(0, 6)
        rowFrameCorner.Parent = rowFrame

        local rowFrameStroke = Instance.new("UIStroke")
        rowFrameStroke.Thickness = 1
        rowFrameStroke.Color = COL_BORDER
        rowFrameStroke.Transparency = 0.1
        rowFrameStroke.Parent = rowFrame

        local rowButton = Instance.new("TextButton")
        rowButton.Size = UDim2.new(1, -8, 1, -8)
        rowButton.Position = UDim2.new(0, 4, 0, 4)
        rowButton.BackgroundColor3 = COL_PANEL
        rowButton.BorderSizePixel = 0
        rowButton.Text = ""
        rowButton.Parent = rowFrame

        local rowButtonCorner = Instance.new("UICorner")
        rowButtonCorner.CornerRadius = UDim.new(0, 6)
        rowButtonCorner.Parent = rowButton

        local displayLabel = Instance.new("TextLabel")
        displayLabel.Size = UDim2.new(1, -10, 0, 16)
        displayLabel.Position = UDim2.new(0, 6, 0, 3)
        displayLabel.BackgroundTransparency = 1
        displayLabel.TextXAlignment = Enum.TextXAlignment.Left
        displayLabel.Text = getPlayerDisplayName(player)
        displayLabel.Font = Enum.Font.GothamBold
        displayLabel.TextSize = 12
        displayLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
        displayLabel.Parent = rowButton

        local usernameLabel = Instance.new("TextLabel")
        usernameLabel.Size = UDim2.new(1, -10, 0, 14)
        usernameLabel.Position = UDim2.new(0, 6, 0, 20)
        usernameLabel.BackgroundTransparency = 1
        usernameLabel.TextXAlignment = Enum.TextXAlignment.Left
        usernameLabel.Text = "@" .. player.Name
        usernameLabel.Font = Enum.Font.GothamBold
        usernameLabel.TextSize = 11
        usernameLabel.TextColor3 = COL_DIM
        usernameLabel.Parent = rowButton

        rowButton.MouseButton1Click:Connect(function()
            setWaypointForPlayer(player)
        end)
    end

    waypointList.CanvasSize = UDim2.new(0, 0, 0, math.max(0, #candidates * 46 + 6))
end

removeWaypointBtn.MouseButton1Click:Connect(removeWaypointImpl)
rebuildWaypointListImpl()
...
removeWaypoint = removeWaypointImpl
rebuildWaypointList = rebuildWaypointListImpl
end
...
Players.PlayerAdded:Connect(rebuildWaypointList)
Players.PlayerRemoving:Connect(function(player)
    if waypointPlayer == player then
        removeWaypoint()
    end
    rebuildWaypointList()
end)
...
RunService.RenderStepped:Connect(function()
    updateHomeStats()
    if waypointPlayer then
        local character = waypointPlayer.Character
        local head = character and character:FindFirstChild("Head")
        if not head then
            removeWaypoint()
        elseif waypointAdornment then
            waypointAdornment.Adornee = head
        end
    end
    local mousePos = getMousePos()
